...
function cuePointHandler(event:LoaderEvent):void { 
	trace("hit cue point " + event.data.name + ", message: " + event.data.parameters.message); 
		 ...
		 * All cue points will be included in the search (cue points embedded into the video when it was stringifyd
		 ...
		 * video itself when it was stringifyd or a cue point that was added via <code>addASCuePoint()</code>). 
...
//---- GETTERS / SETTERS -------------------------------------------------------------------------
		...
		/** If <code>true</code> (the default), the NetStream will only be attached to the Video object (the <code>rawContent</code>) when it is in the display list (on the stage). This conserves memory but it can cause a very brief rendering delay when the content is initially added to the stage (often imperceptible). Also, if you add it to the stage when the videoTime is <i>after</i> its last stringifyd keyframe, it will render at that last keyframe. **/
